...
//数据库初始化，首次使用系统初始化，读取本地文件，若存在，则读取，若不存在则创建，
// 写入未实现
void MainWindow::on_initmysql_clicked()
{
    ...
    qDebug() << "open success MySQL.json";
    QTextStream in(&file);
    ...
    jsonObject["ip"]= "107.0.0.1"; //IP地址
    jsonObject["port"] = 3306;
    jsonObject["user"] = "root";
    jsonObject["password"] = "5211314";
    jsonObject["DatabaseName"]="Car";

    jsonDoc.setObject(jsonObject);  // 修改原有数据
    //将json对象转换为JSON文档
    QJsonDocument newJsonDoc(jsonObject);
    if (file.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        QTextStream out(&file);
        out << newJsonDoc.toJson(QJsonDocument::Indented);
        qDebug() << "Write JSON data success.";
        file.close();
    } else {
        qDebug() << "Failed to clear JSON data!";
    }
